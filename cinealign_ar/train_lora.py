...
from models.wanxgen_lora import WanxgenMulshot
...
class VideoFolder(Dataset):
    ...
    def get_item(self, index):
        ...
        for i, path in enumerate(paths):
            ...
            path = os.path.join(self.cfg.data_root, path)
            # print(path)
...
def worker(cfg):
    ...
    if getattr(cfg, "use_lora", False):
        # 只訓練 LoRA 層（requires_grad=True）
        trainable = [p for p in dit.parameters() if p.requires_grad]
        if cfg.rank == 0:
            print(f"[INFO] LoRA mode: trainable params = {sum(p.numel() for p in trainable)}")
    else:
        # full finetune：全部參數都參與訓練
        trainable = dit.parameters()
        if cfg.rank == 0:
            print("[INFO] Full fine-tune mode: training all parameters")
