...
class FlowDPMSolverMultistepScheduler(SchedulerMixin, ConfigMixin):
    ...
    def index_for_timestep(self, timestep, schedule_timesteps=None):
        ...
        # in case of data accuracy
        tolerance = 1e-5
        matches = torch.isclose(schedule_timesteps, timestep.to(schedule_timesteps.device).to(schedule_timesteps.dtype), atol=tolerance)
        nonzero_indices = matches.nonzero()        
        ...
        pos = 1 if len(nonzero_indices) > 1 else 0

        return nonzero_indices[pos].item()
    ...
    def add_noise(
        ...
        while len(sigma.shape) < original_samples.ndim:  ###
